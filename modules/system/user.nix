...
let
  ...
  combinedKeys =
    if hardwareKeys != [ ] then hardwareKeys else optionals allowSoftFallback softFallbackKeys;
  webauthnConfig = config.frostflake.security.webauthn or { };
  exemptGroup =
    if (webauthnConfig.enable or true) && (webauthnConfig.allowUserOptOut or true) then
      webauthnConfig.exemptGroup or "frostflake-webauthn-exempt"
    else
      null;
